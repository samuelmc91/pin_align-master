...
def auto_submit_button_left(event, image_in_canvas, tk_root):
    global help_image_window
    global auto_start_on_off
    ...
    change_config_file(
        config_file_path, 'DEFAULT_PIXELS_PER_MM', str(pixel_per_mm_in.get()))
    ...
    if rtl:
        if auto_start_on_off:
            ...
            X2 = A + 5
            height = int(roi_height_in.get())
            width = X2 - X1
            info_canvas_top.delete(help_image_window)
            line = image_in_canvas.draw_new_line(X, Y, A, B)
            change_config_file(config_file_path, 'INPUT_ROI_HEIGHT', str(height))
            change_config_file(config_file_path, 'INPUT_ROI_WIDTH', str(width))
            try:
                rise = B - Y
                run = A - X
                m = rise / run
...
if __name__ == '__main__':
    ...
    pixel_per_mm_in = tk.Entry(root, justify='center', width=15)
    pixel_per_mm_in.insert(END, str(DEFAULT_PIXELS_PER_MM))
    info_canvas_top.create_window(310, 315, window=pixel_per_mm_label)
    info_canvas_top.create_window(310, 335, window=pixel_per_mm_in)
    ...
    auto_submit_button.bind(
        "<Button-1>", lambda event, arg=image_in_canvas: auto_submit_button_left(event, image_in_canvas, info_canvas_top))
    ...
    test = info_canvas_bottom.create_window(430, 185, window=tilt_check_bottom_button)
    ...
    #     try:
    #         entry_info = root_children[i].get()
    #         label_text = root_children[i-1].cget('text')
