...
for i in range(len(new_files)):
    ...
    try:
        ...
        if (pos_lines_new == pos_lines_old):
            ...
            new_run_results = os.path.join(new_files[i], f_new[3].split(': ')[-1].split('/')[-1])
            old_run_results = os.path.join(old_files[i], f_old[5].split(': ')[-1].split('/')[-1])
            pos_nc_count += 1
            ...
            # os.system('mv ' + new_run_results + '/* ' + pos_diff_tmp_dir + '/New/')
            # os.system('mv ' + old_run_results + '/* ' + pos_diff_tmp_dir + '/Old/')
