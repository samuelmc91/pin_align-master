...
if __name__ == "__main__":
    ...
    def dir_enter_event(event=None):
        ...
        if not os.path.exists(beamline_dir) or not beam_dir_in.get():
            ...
            beamline_data = Test_Config(beam_dir_in.get())
    ...
    def start_button_click(event=None):
        ...
        if not os.path.exists(beamline_dir) or not beam_dir_in.get():
            messagebox.showerror('Error', 'Directory DOES NOT Exist!')
            return
        pb = ttk.Progressbar(root, orient = HORIZONTAL, length = 265, maximum=test_number_in.get(), mode = 'determinate')
        canvas.create_window(150, 115, window=pb)
        beamline_data = Test_Config(beam_dir_in.get())
        test_index_list = beamline_data.get_index_list(random_choice_in.get(), test_number_in.get())
    ...
    beam_dir_box = Entry(root, width=11, textvariable=beam_dir_in, justify='center')
    ...
    test_number_box = Entry(root, width=11, text=test_number_in, justify='center')
    ...
    start_button = Button(canvas, text='Start', command=lambda: start_button_click())
    canvas.create_window(45, 155, window=start_button)
    cancel_button = Button(canvas, text='Cancel', command=lambda: exit_and_cancel())
