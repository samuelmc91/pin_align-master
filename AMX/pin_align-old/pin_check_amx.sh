...
# PIN_ALIGN_ROOT = Location of the pin_align-master directory holding
# the pin align config
...
. $PIN_ALIGN_ROOT/pin_check_config_amx.sh
#TODO
#When integrated into LSDC the image_in should be taken when unmounting
# the sample when the rotation = 90 degrees
tmp_dir=$PWD/${USER}_pin_align_$$ ### where images used to be saved
#tmp_dir=/tmp # Kun requested images be saved in tmp directory
mkdir $tmp_dir
